  ...
  const fetchComics = async () => {
    try {
      const response = await axios.get(
        "https://www.sankavollerei.com/comic/trending"
      );
      ...
      setComics(processedComics.slice(0, 10));
      setLoading(false);
    } catch (err) {
      setError(err);
      setLoading(false);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`p-3 rounded-full border transition-all duration-300 ${
              canScrollLeft
                ? "bg-white/80 dark:bg-gray-800 border-orange-500/50 text-orange-400 hover:bg-orange-500 hover:text-white hover:scale-110 shadow-lg"
      ...
      <div className="relative">
        {/* Left Gradient Fade */}
        {canScrollLeft && (
          <div className="absolute left-0 top-0 bottom-0 w-16 bg-gradient-to-r from-gray-50 dark:from-[#0a0a0a] to-transparent z-10 pointer-events-none"></div>
        )}

        {/* Right Gradient Fade */}
        {canScrollRight && (
          <div className="absolute right-0 top-0 bottom-0 w-16 bg-gradient-to-l from-gray-50 dark:from-[#0a0a0a] to-transparent z-10 pointer-events-none"></div>
        )}
        ...
        >
          {comics.map((comic, index) => (
            <div
              key={comic.title}
              className="group relative flex-shrink-0 w-48 md:w-56 bg-white/80 dark:bg-gradient-to-b dark:from-gray-800 dark:to-gray-900 backdrop-blur-sm rounded-xl overflow-hidden border border-gray-300 dark:border-gray-700 hover:border-orange-500/50 transition-all duration-300 hover:shadow-2xl hover:shadow-orange-500/20 hover:-translate-y-2 cursor-pointer"
            ...
            >
              ...
              >
                <img
                  ...
                  onError={(e) => {
                    e.target.src =
                      "https://via.placeholder.com/300x450?text=Comic+Cover";
                  }}
                />
              ...
              <div className="p-4">
                <h3
                  className="font-bold text-sm md:text-base line-clamp-2 mb-3 text-gray-900 dark:text-gray-100 group-hover:text-orange-500 dark:group-hover:text-orange-400 transition-colors min-h-[2.5rem] cursor-pointer"
                  onClick={() => handleComicDetail(comic)}
                >
                  {comic.title}
                </h3>
